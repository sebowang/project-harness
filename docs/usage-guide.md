...
1. **接入与勘察**：安装时先用 `-WhatIf` 预览。已有 `AGENTS.md` 时，用户确认是否使用 `-MergeProjectRules`。之后 Agent 通过 `project-onboarding` 从源码和现有文档识别真实目录、模块边界、构建入口、验证命令和风险能力；写入项目配置前先提交 Proposal。
