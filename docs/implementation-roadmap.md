...
4. 仅自动更新未被项目修改的 `managed` 文件；上游移除的文件默认保留为孤儿，显式 `-Prune` 才清理未修改文件。
...
状态：已完成。v1 使用 `harness.lock.json` 基线、`-Update -WhatIf`、事务前预检、备份和失败恢复；项目配置仍由项目所有，不做静默重写。
...
状态：已完成。Standard 提供 `project-onboarding` Skill 与公共工作流，强制 proposal/confirm/apply 分段；确认结果长期写入项目文件，而不是依赖会话记忆。
...
状态：`durable-plan` 已提供可选工作流与 Codex/Claude 入口，未启用时不创建状态文件；其余能力仍按真实项目需求配置，不提供空壳自动化。
